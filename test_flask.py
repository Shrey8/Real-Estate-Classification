...
def home():
    if request.method == 'POST':
        # Make prediction
        df = pd.DataFrame(columns=['price_id1' , 'bedrooms_id1' ,'bathrooms_id1' , 'totalArea_id1' , 
                                                'price_id2' , 'bedrooms_id2' ,'bathrooms_id2' , 'totalArea_id2' ,
                                                'apartment_dummy_1' ,'house_dummy_1' , 'plot_dummy_1' , 'investment_dummy_1' ,  'other_dummy_1' ,
                                               'apartment_dummy_2' ,'house_dummy_2' , 'plot_dummy_2' , 'investment_dummy_2' ,  'other_dummy_2' ,
                                            'title_id1' , 'title_id2' , 'description_id1' , 'description_id2'])
        
        values = []
        for i in df:
            values.append(request.form.get(i))
                  
        df.loc[0] = values
        df = df.astype(convert_dict) 
        pred = main_pipeline.predict(df)
        return render_template('index.html', sentiment=pred[0])
